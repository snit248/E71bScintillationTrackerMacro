void NPeakSearch_plot_hodoscope(){

    TString ifilename = "./data/0620bv5552HNth2DAC613u549dYNU26.root";  //読み込むファイル名(All)
    //TString ifilename = "../merge/X1_"+unixtime+"_conv.root";  //読み込むファイル名(module)

    TFile *file = new TFile(ifilename); //ファイルを読み込む
    TTree *tree = (TTree*)file->Get("Data");  //TTreeの作成
    ...
    ////Npeak.datに書き込んで出力////
    ofstream fout("./rc/Npeak.dat");  //All
    ...
    //for(Int_t i=192; i<208; i++){
        ...
        ////hの定義////
        //h[i] = new TH1F(Form("h%d",i), Form("h%d",i), 400, 0, 4000);
        h[i] = new TH1F(Form("h%d",i), Form("h%d",i), 200, 650, 1050); //origin
        //h[i] = new TH1F(Form("h%d",i), Form("h%d",i), 100, 950, 1400);
        ...
        corrangemax = (Int_t)xpeaks[0]+85;  //範囲の最大値をpedestal+85に設定
